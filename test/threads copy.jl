...
function build_deriv_loops_clean(d, w, p, N)
    ...
    if d == 1
        ...
        if p == 1
            ...
            # Fast contiguous write to the transposed [a1, ap] matrix
            q1_write = :( (results[$p][1])[a1, $ap] += val * $(w[1]) )

            # Shared scalar reductions for all q > 1
            accumulators = [:( (results[$p][$q])[$ap, $(Symbol("a", q))] += s_shared * $(w[q]) )
                            for q in 2:N if q != p]
            ...
            if isempty(accumulators)
                ...
                return quote
                    ...
                    @simd ivdep for a1 in axes(pay_p, 1)
                        val = pay_p[$(a_all...)]
                        $q1_write
                        s_shared += val * pi[1][a1] # Contiguous shared read
                    end
                    $(accumulators...)
                end
            end
    ...
    # Generate the next iteration's weights
    next_w = Any[is_active(q) ? Symbol("w_d", d, "_q", q) : w[q] for q in 1:N]

    # Generate the constant assignments only for active dimensions
...
unilateral_derivatives_optimal!(dudpi1_transposed, Us, pi);
...
b = @benchmark unilateral_derivatives_optimal!($dudpi1_transposed, $Us, $pi)
