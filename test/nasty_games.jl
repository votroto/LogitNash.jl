...
function test_unscaled_games_succeed_directly_or_after_scaling(dimensions, generator; attempts=20)
    for _ in 1:attempts
        ...
        _, status_nasty = @test_nowarn nash(payoffs_nasty; stop_iters=500, stop_t=1e6, stop_eps=1e-6)
        if !status_nasty.stall
            continue
        end
        # Found a stalling game
        payoffs_scaled = matrix_rescale.(payoffs_nasty)
        _, status_scaled = nash(payoffs_scaled; stop_iters=500, stop_t=1e6, stop_eps=1e-6)
